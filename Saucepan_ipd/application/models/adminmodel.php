...
class AdminModel extends CI_Model{
	...
	function checkCoupon_username($name){
		$sql="select * from ".DB_PRE()."coupon_list WHERE coupon_code='$name'";
		$query=$this->db->query($sql);
		if($query->num_rows()>0){
			return $query->row_array();
		}else{
	...
	function checkCoupon($name, $pwd){
		$sql="select * from ".DB_PRE()."coupon_list WHERE coupon_code = '$name' AND coupon_password='$pwd'";
		$query=$this->db->query($sql);
		if($query->num_rows()>0){
			return $query->row_array();
		}else{
