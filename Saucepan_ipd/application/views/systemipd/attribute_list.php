        ...
        <div class="">
            <input class="input order_no" name="order_no"
                   ...
                   placeholder="<?php if ($this->langtype == '_ch') {
                       ...
                       echo 'Attribute Name / Category Name / Attribute No.';
                   } ?>: XXX"/>
        </div>
...
<script>
    ...
    function load(type){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if(data.status == 1){
                    $("#table_body").html('');
                    var attributes = data.response;
                    var tableHtml = '';
                    var modalHtml = '';
                    for(var i = 0 ; i < attributes.length ; i++ ) {
                        var chkstatus = attributes[i].pa_status=='Online'?'checked':'';
                        tableHtml += "<tr>" +
                            "<td>"+attributes[i].product_attribute_id +"</td>" +
                            // "<td><img src='"+baseUrl+products[i].product_image +"' class = 'product_image' /></td>" +
                            "<td>"+attributes[i].lang_attribute_title +"</td>" +
                            "<td>"+attributes[i].original_price +"</td>" +
                            "<td>"+attributes[i].lang_category_title + "(" + attributes[i].product_category_title_en+")" +"</td>" +
                            "<td>"+attributes[i].pa_status +"</td>" +
                            '<td><label class="switch">'+
                            '<input type="checkbox" '+chkstatus+' onchange = updateAttribtueStatus('+attributes[i].product_attribute_id+',"'+chkstatus+'")>'+
    ...
    function updateAttribtueStatus(id,status){
        ...
        $.ajax({
            type: 'POST',
            url: "<?php echo api_url();?>"+"update_sp_attribute_status",
            // cache:false,
            dataType: "json",
            data: data,
            // processData: false,
