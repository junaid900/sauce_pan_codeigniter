...
<script type="text/javascript" src='<?php echo CDN_URL();?>themes/default/js/admin/admin_designerone.js?date=<?php echo CACHE_USETIME()?>'></script>
...
<form method="post">
	<table class="gksel_normal_tabpost">
		<tr>
			...
			<td>
				<script>
					function toviewdesigneroneoriginal(path){
				...
				<div class="img_gksel_show" id="img1_gksel_show">
					<div style="float:left;">
						<?php 
							$img1_gksel = '';
							if(file_exists($designeroneinfo['designerone_pic_800']) && $designeroneinfo['designerone_pic_800']!=""){
								echo '<img style="float:left;max-width:400px;max-height:400px;" src="'.base_url().$designeroneinfo['designerone_pic_800'].'" />';
							}
							
							if(file_exists($designeroneinfo['designerone_pic_original']) && $designeroneinfo['designerone_pic_original']!=""){
								$img1_gksel = $designeroneinfo['designerone_pic_original'];
							}
						?>
					</div>
					<div style="float:left;margin-left:10px;">
						<?php if(file_exists($designeroneinfo['designerone_pic_original']) && $designeroneinfo['designerone_pic_original']!=""){?>
							<a href="javascript:;" onclick="toviewdesigneroneoriginal('<?php echo $designeroneinfo['designerone_pic_original']?>')"><?php if($this->langtype == '_ch'){echo '查看原图';}else{echo 'View original photo';}?></a>
						<?php }?>
					</div>
				</div>
		...
		<tr style="display:none;">
			<td align="right" width="150">优势</td>
			<td align="left">
				<input type="text" name="designerone_advantage" style="width:300px;" value="<?php echo $designeroneinfo['designerone_advantage']?>"/>
				<div class="tipsgroupbox"></div>
			</td>
		</tr>
		<tr style="display:none;">
			<td align="right" width="150">座右铭</td>
			<td align="left">
				<input type="text" name="designerone_motto" style="width:300px;" value="<?php echo $designeroneinfo['designerone_motto']?>"/>
				<div class="tipsgroupbox"></div>
			</td>
		</tr>
		<tr>
		...
		<?php for ($lc = 0; $lc < count($lancodelist); $lc++) {?>
			<tbody>
				<tr>
					...
					<td align="left">
						<input type="text" name="designerone_name<?php echo $lancodelist[$lc]['langtype']?>" style="width:300px;" value="<?php echo $designeroneinfo['designerone_name'.$lancodelist[$lc]['langtype']]?>"/>
						<div class="tipsgroupbox"></div>
					</td>
				</tr>
				<tr>
					<td align="right" width="150"><?php if($this->langtype == '_ch'){echo '职位';}else{echo 'Position';}?> <?php if(count($lancodelist) != 1){echo '('.$lancodelist[$lc]['langname'].')';}?></td>
					<td align="left">
						<input type="text" name="designerone_position<?php echo $lancodelist[$lc]['langtype']?>" style="width:300px;" value="<?php echo $designeroneinfo['designerone_position'.$lancodelist[$lc]['langtype']]?>"/>
						<div class="tipsgroupbox"></div>
					</td>
				</tr>
				<tr>
					<td align="right" width="150"><?php echo lang('dz_designerone_description')?> <?php if(count($lancodelist) != 1){echo '('.$lancodelist[$lc]['langname'].')';}?></td>
					<td align="left">
						<script id="designerone_description<?php echo $lancodelist[$lc]['langtype']?>" type="text/plain" style="width:800px;height:300px;"><?php echo debaseurlcontent($designeroneinfo['designerone_description'. $lancodelist[$lc]['langtype']])?></script>
						<script type="text/javascript">
							var designerone_description<?php echo $lancodelist[$lc]['langtype']?> = UE.getEditor('designerone_description<?php echo $lancodelist[$lc]['langtype']?>');
						</script>
					</td>
				</tr>
			</tbody>
		...
		<tr style="display:none;">
			...
			<td align="left">
				<input name="status" type="checkbox" defaultvalue="0" class="mgc-switch" value="1" <?php if($designeroneinfo['status'] == 1){echo 'checked';}?>/>
			</td>
		</tr>
		<tr>
			<td>
			...
			<td align="left">
				<div class="gksel_btn_action_on" onclick="tosave_designeroneinfo(<?php echo $designeroneinfo['designerone_id']?>)"><?php echo lang('cy_save')?></div>
			</td>
		</tr>
	</table>
</form>
...
$(document).ready(function(){
	...
	//商品信息---保存
	function tosave_designeroneinfo(designerone_id){
		var lancodelist = getlancodelist();
		if(isajaxsaveing == 0){
			//具体点击的按钮
			actionsubmit_button = $('div[onclick="tosave_designeroneinfo('+designerone_id+')"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
			...
			//----定义的字段------START
				...
				GETOBJ[GETOBJ_num]['field_realname'] = 'designerone_pic_original';
				GETOBJ[GETOBJ_num]['field_type'] = "image";
				GETOBJ[GETOBJ_num]['field_CMNAME'] = L['dz_designerone_picture'];

				GETOBJ_num = GETOBJ_num + 1;
				GETOBJ[GETOBJ_num] = new Array();
				GETOBJ[GETOBJ_num]['field_name'] = 'designerone_advantage';
				GETOBJ[GETOBJ_num]['field_realname'] = 'designerone_advantage';
				GETOBJ[GETOBJ_num]['field_type'] = "input";
				GETOBJ[GETOBJ_num]['field_CMNAME'] = '';

				GETOBJ_num = GETOBJ_num + 1;
				GETOBJ[GETOBJ_num] = new Array();
				GETOBJ[GETOBJ_num]['field_name'] = 'designerone_motto';
				GETOBJ[GETOBJ_num]['field_realname'] = 'designerone_motto';
				GETOBJ[GETOBJ_num]['field_type'] = "input";
				GETOBJ[GETOBJ_num]['field_CMNAME'] = '';
			//----定义的字段------END
			...
			//----定义多语言的字段------START
				...
				GETLANGOBJ[GETLANGOBJ_num]['field_name'] = 'designerone_name';
				GETLANGOBJ[GETLANGOBJ_num]['field_type'] = "input";
				GETLANGOBJ[GETLANGOBJ_num]['field_CMNAME'] = L['dz_designerone_name'];

				GETLANGOBJ_num = GETLANGOBJ_num + 1;
				GETLANGOBJ[GETLANGOBJ_num] = new Array();
				GETLANGOBJ[GETLANGOBJ_num]['field_name'] = 'designerone_position';
				GETLANGOBJ[GETLANGOBJ_num]['field_type'] = "input";
				GETLANGOBJ[GETLANGOBJ_num]['field_CMNAME'] = '';

				GETLANGOBJ_num = GETLANGOBJ_num + 1;
				GETLANGOBJ[GETLANGOBJ_num] = new Array();
				GETLANGOBJ[GETLANGOBJ_num]['field_name'] = 'designerone_description';
				GETLANGOBJ[GETLANGOBJ_num]['field_type'] = "ueditor";
				GETLANGOBJ[GETLANGOBJ_num]['field_CMNAME'] = L['dz_designerone_description'];
			//----定义多语言的字段------END
			...
			if(returnOBJ.ispass == 1){
				$.post(baseurl+'index.php/admins/designerone/edit_designerone/'+designerone_id, postOBJ, function (data){
					var obj = eval( "(" + data + ")" );
