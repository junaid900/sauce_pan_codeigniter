...
<script>
	function tochooseparentcategory_id(category_id){
		$.post(baseurl+'index.php/admins/article/togetsubcategory_select/'+category_id, function (data){
			var obj = eval( "(" + data + ")" );
	...
	<tr>
		<td>
			<div class="searcharea">
				<form action = "<?php echo base_url().'index.php/admins/article/index'?>" method="get">
					<?php 
...
// 						$fenleilist = $this->ArticleModel->getarticlecategorylist($con);
...
<table class="gksel_normal_tabaction" style="display: none;">
	<tr>
		<td>
			<div class="searcharea">
				<a href="<?php echo base_url().'index.php/admins/article/toadd_article'?>">
					<font class="nav_on">
						<font class="plus">
						...
						<span><?php if($this->langtype == '_ch'){echo '添加活动';}else{echo 'Add Article';}?></span>
					</font>
				</a>
			</div>
		</td>
...
<table class="gksel_normal_tablist">
	...
	<tbody>
		<?php if(isset($articlelist)){for ($i = 0; $i < count($articlelist); $i++) {?>
			<tr style="<?php if($articlelist[$i]['status'] == 0){echo 'opacity:0.2;';}?>">
				<td align="center"><?php echo ($i+1)?></td>
				<td align="center">
					<?php if($articlelist[$i]['article_pic_200']!=""){?>
						<img style="max-width:70px;max-height: 70px;" src="<?php echo CDN_URL().$articlelist[$i]['article_pic_200']?>"/>
					<?php }else{?>
				...
				<td align="center"><?php echo date("Y-m-d",$articlelist[$i]['datepicker']);?></td>
				<!--
				<td>
					<div style="float:left;width: 100%;<?php if($articlelist[$i]['article_price_regular'] == 0){echo 'color:#CCC;';}?>">
						<?php echo '&yen;'.number_format($articlelist[$i]['article_price_regular'], 2, ".", ",");?>
					</div>
				</td>
				-->
				<td align="center"><?php echo date('Y-m-d', $articlelist[$i]['created']).'<br />'.date('H:i:s', $articlelist[$i]['created'])?></td>
				<td align="center"><?php echo $articlelist[$i]['edited_author']?></td>
				<td align="center">
					<?php 
						if($articlelist[$i]['status'] == 1){
							echo lang('cy_online');
						}else{
				...
				<td align="center">
					<div style="float:right;margin-right:50px;">
						<?php 
							$con = array('article_id'=>$articlelist[$i]['article_id']);
...
// 							echo '<a href="'.base_url().'index.php/admins/article/picturelist/'.$articlelist[$i]['article_id'].'?backurl='.$current_url_encode.'" class="gksel_btn_action_on">'.$text.'</a>';
						
							echo '<a href="'.base_url().'index.php/admins/article/toedit_article/'.$articlelist[$i]['article_id'].'?backurl='.$current_url_encode.'" class="gksel_btn_action_on">'.lang('cy_edit').'</a>';
								
							echo '<a onclick="todel_article('.$articlelist[$i]['article_id'].', \''.$articlelist[$i]['article_name'.$this->langtype].'\')" href="javascript:;" class="gksel_btn_action_on" style="display:none">'.lang('cy_delete').'</a>';
						?>
					</div>
				</td>
			</tr>
...
function todel_article(id, name){
	...
	var subtitle = '';
	del_url = encodeURI(baseurl+"index.php/admins/article/del_article/"+id);
