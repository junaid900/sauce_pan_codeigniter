...
<form method="post">
	<table class="gksel_normal_tabpost">
		<tr>
			...
			<td>
				<script>
					function toviewserviceoriginal(path){
				...
				<div class="img_gksel_show" id="img1_gksel_show">
					<div style="float:left;">
						<?php 
							$img1_gksel = '';
							if(file_exists($serviceinfo['service_pic_800']) && $serviceinfo['service_pic_800']!=""){
								echo '<img style="float:left;max-width:400px;max-height:400px;" src="'.base_url().$serviceinfo['service_pic_800'].'" />';
							}
							
							if(file_exists($serviceinfo['service_pic_original']) && $serviceinfo['service_pic_original']!=""){
								$img1_gksel = $serviceinfo['service_pic_original'];
							}
						?>
					</div>
					<div style="float:left;margin-left:10px;">
						<?php if(file_exists($serviceinfo['service_pic_original']) && $serviceinfo['service_pic_original']!=""){?>
							<a href="javascript:;" onclick="toviewserviceoriginal('<?php echo $serviceinfo['service_pic_original']?>')"><?php if($this->langtype == '_ch'){echo '查看原图';}else{echo 'View original photo';}?></a>
						<?php }?>
					</div>
				</div>
							...
							echo '<optgroup label="'.$fenleilist[$aaa]['category_name'.$this->langtype].'">';
								...
								if(!empty($articlecategorylist)){
									for ($i = 0; $i < count($articlecategorylist); $i++) {
										$ischecked = '';
										if($articlecategorylist[$i]['category_id'] == $serviceinfo['subcategory_id']){
											$ischecked = 'selected';
										}
		...
		<?php for ($lc = 0; $lc < count($lancodelist); $lc++) {?>
			<tbody>
				<tr>
					...
					<td align="left">
						<input type="text" name="service_name<?php echo $lancodelist[$lc]['langtype']?>" style="width:300px;" value="<?php echo $serviceinfo['service_name'.$lancodelist[$lc]['langtype']]?>"/>
        			...
        			<td align="right" width="150"><?php if($this->langtype == '_ch'){echo '时间';}else{echo 'Time';}?></td>
        			  ...
        			  <td>
        			     <div class="form-group float-left w140" style="margin-bottom:0;">
                              <input type="text" name="datepicker"  id="datetimepicker3" class="form-control" value="<?php echo date('Y-m-d', $serviceinfo['datepicker'])?>"/>
				...
				<tr>
					<td align="right" width="150"><?php echo lang('dz_service_description')?> <?php if(count($lancodelist) != 1){echo '('.$lancodelist[$lc]['langname'].')';}?></td>
					<td align="left">
						<script id="service_description<?php echo $lancodelist[$lc]['langtype']?>" type="text/plain" style="width:800px;height:300px;"><?php echo debaseurlcontent($serviceinfo['service_description'. $lancodelist[$lc]['langtype']])?></script>
		...
		<tr>
			...
			<td align="left">
				<input name="status" type="checkbox" defaultvalue="0" class="mgc-switch" value="1" <?php if($serviceinfo['status'] == 1){echo 'checked';}?>/>
			</td>
		</tr>
		<tr>
			<td>
			...
			<td align="left">
				<div class="gksel_btn_action_on" onclick="tosave_serviceinfo(<?php echo $serviceinfo['service_id']?>)"><?php echo lang('cy_save')?></div>
			</td>
		</tr>
	</table>
</form>
...
$(document).ready(function(){
	...
	//商品信息---保存
	function tosave_serviceinfo(service_id){
		var lancodelist = getlancodelist();
		if(isajaxsaveing == 0){
			//具体点击的按钮
			actionsubmit_button = $('div[onclick="tosave_serviceinfo('+service_id+')"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
			...
			if(returnOBJ.ispass == 1){
				$.post(baseurl+'index.php/admins/service/edit_service/'+service_id, postOBJ, function (data){
					var obj = eval( "(" + data + ")" );
