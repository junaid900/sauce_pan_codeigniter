	...
	<tr>
		<td>
			<div class="searcharea">
				<form action = "<?php echo base_url().'index.php/admins/event/categorylist'?>" method="get">
...
<table class="gksel_normal_tabaction">
	<tr>
		<td>
			<div class="searcharea">
				<a href="<?php echo base_url().'index.php/admins/event/toadd_event_category'?>"><font class="nav_on"><img class="plus" src="<?php echo base_url().'themes/default/images/plus.png'?>"/> <?php if($this->langtype == '_ch'){echo '添加分类';}else{echo 'Add Category';}?></font></a>
			</div>
		</td>
	</tr>
</table>
<table class="gksel_normal_tablist">
	<thead>
		<tr>
			...
			<td><p>&nbsp;&nbsp;&nbsp;<?php if($this->langtype == '_ch'){echo '分类名称';}else{echo 'Category Name';}?></p></td>
						...
						<td width="350" align="center" style="padding:0px;">
							<div style="float:right;">
								<?php 
									...
									$countsubcategory = $this->EventModel->geteventcategorylist($con, 1);
									...
									echo '<a href="'.base_url().'index.php/admins/event/subcategorylist/'.$categorylist[$i]['category_id'].'?backurl='.$current_url_encode.'" class="gksel_btn_action_on">'.$text.'</a>';
									echo '<a href="'.base_url().'index.php/admins/event/toedit_eventcategory/'.$categorylist[$i]['category_id'].'?backurl='.$current_url_encode.'" class="gksel_btn_action_on">'.lang('cy_edit').'</a>';
									if($countsubcategory > 0){
										echo '<a href="javascript:;" class="gksel_btn_action_off">'.lang('cy_delete').'</a>';
									}else{
										echo '<a onclick="todel_eventcategory('.$categorylist[$i]['category_id'].', \''.$categorylist[$i]['category_name'.$this->langtype].'\')" href="javascript:;" class="gksel_btn_action_on">'.lang('cy_delete').'</a>';
									}
								?>
							</div>
						</td>
	...
	jQuery(function($) {
		...
		$('#tasks').sortable({
			...
			stop: function( event,ui) {//just for Chrome!!!! so that dropdowns on items don't appear below other items after being moved
				...
				$.post(baseurl+'index.php/admins/event/editcategory_sort',{idarr:idarr,eventrot:eventrot},function (data){
