...
<script type="text/javascript" src='<?php echo CDN_URL();?>themes/default/js/admin/admin_partner.js?date=<?php echo CACHE_USETIME()?>'></script>
...
<form method="post">
	<table class="gksel_normal_tabpost">
		<tr>
			...
			<td>
				<script>
					function toviewpartneroriginal(path){
				...
				<div class="img_gksel_show" id="img1_gksel_show">
					<div style="float:left;">
						<?php 
							$img1_gksel = '';
							if(file_exists($partnerinfo['partner_pic_800']) && $partnerinfo['partner_pic_800']!=""){
								echo '<img style="float:left;max-width:400px;max-height:400px;" src="'.base_url().$partnerinfo['partner_pic_800'].'" />';
							}
							
							if(file_exists($partnerinfo['partner_pic_original']) && $partnerinfo['partner_pic_original']!=""){
								$img1_gksel = $partnerinfo['partner_pic_original'];
							}
						?>
					</div>
					<div style="float:left;margin-left:10px;">
						<?php if(file_exists($partnerinfo['partner_pic_original']) && $partnerinfo['partner_pic_original']!=""){?>
							<a href="javascript:;" onclick="toviewpartneroriginal('<?php echo $partnerinfo['partner_pic_original']?>')"><?php if($this->langtype == '_ch'){echo '查看原图';}else{echo 'View original photo';}?></a>
						<?php }?>
					</div>
				</div>
		...
		<?php for ($lc = 0; $lc < count($lancodelist); $lc++) {?>
			<tbody>
				<tr>
					...
					<td align="left">
						<input type="text" name="partner_name<?php echo $lancodelist[$lc]['langtype']?>" style="width:300px;" value="<?php echo $partnerinfo['partner_name'.$lancodelist[$lc]['langtype']]?>"/>
						<div class="tipsgroupbox"></div>
					</td>
				</tr>
				<tr style="display:none;">
					<td align="right" width="150"><?php echo lang('dz_partner_description')?> <?php if(count($lancodelist) != 1){echo '('.$lancodelist[$lc]['langname'].')';}?></td>
					<td align="left">
						<script id="partner_description<?php echo $lancodelist[$lc]['langtype']?>" type="text/plain" style="width:800px;height:300px;"><?php echo debaseurlcontent($partnerinfo['partner_description'. $lancodelist[$lc]['langtype']])?></script>
						<script type="text/javascript">
							var partner_description<?php echo $lancodelist[$lc]['langtype']?> = UE.getEditor('partner_description<?php echo $lancodelist[$lc]['langtype']?>');
						</script>
					</td>
				</tr>
				<tr>
		...
		<tr>
			...
			<td align="left">
				<input name="status" type="checkbox" defaultvalue="0" class="mgc-switch" value="1" <?php if($partnerinfo['status'] == 1){echo 'checked';}?>/>
			</td>
		</tr>
		<tr>
			<td>
			...
			<td align="left">
				<div class="gksel_btn_action_on" onclick="tosave_partnerinfo(<?php echo $partnerinfo['partner_id']?>)"><?php echo lang('cy_save')?></div>
			</td>
		</tr>
	</table>
</form>
...
$(document).ready(function(){
	...
	//商品信息---保存
	function tosave_partnerinfo(partner_id){
		var lancodelist = getlancodelist();
		if(isajaxsaveing == 0){
			//具体点击的按钮
			actionsubmit_button = $('div[onclick="tosave_partnerinfo('+partner_id+')"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
			...
			//----定义的字段------START
				...
				GETOBJ[GETOBJ_num]['field_realname'] = 'partner_pic_original';
				GETOBJ[GETOBJ_num]['field_type'] = "image";
				GETOBJ[GETOBJ_num]['field_CMNAME'] = L['dz_partner_picture'];
			//----定义的字段------END
			...
			//----定义多语言的字段------START
				...
				GETLANGOBJ[GETLANGOBJ_num]['field_name'] = 'partner_name';
				GETLANGOBJ[GETLANGOBJ_num]['field_type'] = "input";
				GETLANGOBJ[GETLANGOBJ_num]['field_CMNAME'] = L['dz_partner_name'];

				GETLANGOBJ_num = GETLANGOBJ_num + 1;
				GETLANGOBJ[GETLANGOBJ_num] = new Array();
				GETLANGOBJ[GETLANGOBJ_num]['field_name'] = 'partner_description';
				GETLANGOBJ[GETLANGOBJ_num]['field_type'] = "ueditor";
				GETLANGOBJ[GETLANGOBJ_num]['field_CMNAME'] = L['dz_partner_description'];
			//----定义多语言的字段------END
			...
			if(returnOBJ.ispass == 1){
				$.post(baseurl+'index.php/admins/partner/edit_partner/'+partner_id, postOBJ, function (data){
					console.log(data);
					var obj = eval( "(" + data + ")" );
