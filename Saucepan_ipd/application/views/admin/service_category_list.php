...
<table class="gksel_normal_tabaction">
	<tr>
		<td>
			<div class="searcharea">
				<a href="<?php echo base_url().'index.php/admins/service/toadd_service_category'?>"><font class="nav_on"><img class="plus" src="<?php echo base_url().'themes/default/images/plus.png'?>"/> <?php if($this->langtype == '_ch'){echo '添加分类';}else{echo 'Add Category';}?></font></a>
			</div>
		</td>
	</tr>
</table>
						...
						<td width="350" align="center" style="padding:0px;">
							<div style="float:right;">
								<?php 
									...
									$countsubcategory = $this->ServiceModel->getservicecategorylist($con, 1);
									...
									echo '<a href="'.base_url().'index.php/admins/service/subcategorylist/'.$categorylist[$i]['category_id'].'?backurl='.$current_url_encode.'" class="gksel_btn_action_on">'.$text.'</a>';
									echo '<a href="'.base_url().'index.php/admins/service/toedit_servicecategory/'.$categorylist[$i]['category_id'].'?backurl='.$current_url_encode.'" class="gksel_btn_action_on">'.lang('cy_edit').'</a>';
									if($countsubcategory > 0){
										echo '<a href="javascript:;" class="gksel_btn_action_off">'.lang('cy_delete').'</a>';
									}else{
										echo '<a onclick="todel_servicecategory('.$categorylist[$i]['category_id'].', \''.$categorylist[$i]['category_name'.$this->langtype].'\')" href="javascript:;" class="gksel_btn_action_on">'.lang('cy_delete').'</a>';
									}
								?>
							</div>
						</td>
