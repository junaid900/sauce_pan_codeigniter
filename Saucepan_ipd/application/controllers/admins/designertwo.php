...
class Designertwo extends CI_Controller{
	...
	//设计师列表
	function index(){
		$this->session->set_userdata('menu','designertwo');
		...
		$data['designertwolist']=$this->DesignertwoModel->getdesignertwolist($con);
		$data['count']=$this->DesignertwoModel->getdesignertwolist($con,1);
		$url = base_url().'index.php/admins/designertwo/index?keyword='.$keyword;
		$data['fy'] = fy_backend($data['count'],$row,$url,$data['page'],5,2);
		
		$this->load->view('admin/designertwo_list',$data);
	}
	
	//添加设计师
	function toadd_designertwo(){
		//跳转到列表页面
		$backurl = base64_encode(base_url().'index.php/admins/designertwo/index');
		if($backurl!=""){
			$backurl=str_replace('slash_tag','/',$backurl);
			if(base64_decode($backurl)!=""){
				$decodebackurl = base64_decode(str_replace(" ", "+", $backurl));
			}else{
				$decodebackurl = base_url().'index.php/admins/designertwo/index';
			}
		}else{
			$decodebackurl = base_url().'index.php/admins/designertwo/index';
		}
		...
		if($this->langtype == '_ch'){
			...
			$data['url'] = '<a href="'.$decodebackurl.'">Manage designertwo</a> &gt; Add designertwo';
		}
		
		
		$this->load->view('admin/designertwo_add',$data);
	}
	//添加设计师 ------- 处理方法
	function add_designertwo(){
		...
		//自动生成 shorturl -- 开始
		if(isset($designertwo_name_en) && $designertwo_name_en != ''){
			$shorturl = $this->PinyinModel->getshorturlPY($designertwo_name_en, '', 1000);
		}else if($designertwo_name_ch && $designertwo_name_ch != ''){
			$shorturl = $this->PinyinModel->getshorturlPY($designertwo_name_ch, '', 1000);
		}else{
			$shorturl = '';
		}
		//自动生成 shorturl -- 结束
		...
		$designertwo_id = $this->DesignertwoModel->add_designertwo($arr);
		...
		$arr_pic = autotofilepath('designertwo', $arr_pic);
		if(!empty($arr_pic)){
			...
			$old_pic = $arr_pic['designertwo_pic_original'];
	...
	//修改设计师
	function toedit_designertwo($designertwo_id){
		//跳转到列表页面
		...
		if($this->langtype == '_ch'){
			...
			$data['url'] = '<a href="'.$decodebackurl.'">Manage designertwo</a> &gt; Edit designertwo';
		}
		
		$data['designertwoinfo']=$this->DesignertwoModel->getdesignertwoinfo($designertwo_id);
		$this->load->view('admin/designertwo_edit',$data);
	}
	//修改设计师 ------- 处理方法
	function edit_designertwo($designertwo_id){
		$olddesignertwoinfo = $this->DesignertwoModel->getdesignertwoinfo($designertwo_id);
		...
		$this->DesignertwoModel->edit_designertwo($designertwo_id, $arr);
	...
	function del_designertwo($designertwo_id){
		$this->DesignertwoModel->del_designertwo($designertwo_id);
	}

	//设计师列表 -- 排序功能
	function editdesignertwo_sort(){
		$idarr=$this->input->post('idarr');
		$newsrot=$this->input->post('newsrot');
		if(!empty($idarr)){
			for($i=0;$i<count($idarr);$i++){
				$arr = array('sort'=>$newsrot[$i]);
				$this->DesignertwoModel->edit_designertwo($idarr[$i], $arr);
			}
		}
	}
