<?php 
    $details = $this->db->get_where('news_sub_tree_category', array('news_sub_tree_category_id'=>$param1))->row_array();  
    $labels =  $this->db->get('news_sub_category')->result_array(); 
?>

<div class="row">
	<div class="col-md-12"> 
		<div class="card">
			<h4 class="card-header mt-0"><i class="fa fa-pencil"></i><?php echo get_phrase('edit_news_sub_tree_category'); ?></h4>
			<div class="card-body">
			<form action="<?php echo base_url().strtolower($this->session->userdata('directory')). '/manage_news_sub_tree_category/edit/'.$param1; ?>" method="post"  enctype ='multipart/form-data'>
					    <div class="form-group row">
            				<label class="col-sm-4 col-form-label"><?php echo get_phrase('news_sub_category'); ?></label>
            				<div class="col-sm-8">
            					<select id="category_id" name="sub_category_id" required="required" class="form-control">
            						<option value="" disabled selected>Please select</option>
            						<?php foreach($labels as $lbl){ ?>
            						<option value="<?php echo $lbl['news_sub_category_id']; ?>" <?php if($details['news_sub_category_id'] == $lbl['news_sub_category_id']) echo 'selected'; ?>> <?php echo $lbl['en_title']; ?> -- <?php echo $lbl['ch_title']; ?></option>
            						<?php } ?>
            					</select>
            				</div>
            			</div>
