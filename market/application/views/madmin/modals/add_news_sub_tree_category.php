<?php $labels =  $this->db->get('news_sub_category')->result_array(); ?>
<div class="card">
	<h3 class="card-header mt-0"><i class="fa fa-plus"></i> <?php echo get_phrase('add_news_sub_tree_category'); ?></h3>
	<div class="card-body">
		<form action="<?php echo base_url().strtolower($this->session->userdata('directory')). '/manage_news_sub_tree_category/add'; ?>" method="post"  enctype ='multipart/form-data'>
			...
			<div class="form-group row">
				...
				<div class="col-sm-8">
					<select id="sub_category_id" name="sub_category_id" required="required" class="form-control">
						...
						<option value="<?php echo $lbl['news_sub_category_id']; ?>"><?php echo $lbl['en_title']; ?> -- <?php echo $lbl['ch_title']; ?></option>
						<?php } ?>
					</select>
				</div>
			</div>
