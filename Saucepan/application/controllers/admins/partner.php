...
class Partner extends CI_Controller{
	...
	//合作伙伴列表
	function index(){
		$this->session->set_userdata('menu','partner');
		...
		$data['partnerlist']=$this->PartnerModel->getpartnerlist($con);
		$data['count']=$this->PartnerModel->getpartnerlist($con,1);
		$url = base_url().'index.php/admins/partner/index?keyword='.$keyword;
	...
	//添加合作伙伴
	function toadd_partner(){
		//跳转到列表页面
		$backurl = base64_encode(base_url().'index.php/admins/partner/index');
		if($backurl!=""){
			$backurl=str_replace('slash_tag','/',$backurl);
			if(base64_decode($backurl)!=""){
				$decodebackurl = base64_decode(str_replace(" ", "+", $backurl));
			}else{
				$decodebackurl = base_url().'index.php/admins/partner/index';
			}
		}else{
			$decodebackurl = base_url().'index.php/admins/partner/index';
		}
		...
		if($this->langtype == '_ch'){
			$data['url'] = '<a href="'.$decodebackurl.'">管理合作伙伴</a> &gt; 添加合作伙伴';
		}else{
			$data['url'] = '<a href="'.$decodebackurl.'">Manage partner</a> &gt; Add partner';
		}
	...
	function add_partner(){
		...
		//自动生成 shorturl -- 开始
		if(isset($partner_name_en) && $partner_name_en != ''){
			$shorturl = $this->PinyinModel->getshorturlPY($partner_name_en, '', 1000);
		}else if(isset($partner_name_ch) && $partner_name_ch != ''){
			$shorturl = $this->PinyinModel->getshorturlPY($partner_name_ch, '', 1000);
		}else{
			$shorturl = '';
		}
		//自动生成 shorturl -- 结束
		...
		$partner_id = $this->PartnerModel->add_partner($arr);
		...
		$arr_pic = autotofilepath('partner', $arr_pic);
		if(!empty($arr_pic)){
			...
			$old_pic = $arr_pic['partner_pic_original'];
			$check_oldpic = explode('/',$old_pic);
			$old_arr = explode('.', $old_pic);
			$pic_type = end($old_arr);
			
			$copy_Thumb = $uploaddir.'/'.$section.'_1000x798_'.date('Y_m_d_H_i_s').'.'.$pic_type;
			$res = copy($old_pic, $copy_Thumb);
			$this->app->my_image_resize ( $copy_Thumb, $copy_Thumb, 1000, 798 );
			$arr_pic['partner_pic']=$copy_Thumb;
			

			$copy_Thumb = $uploaddir.'/'.$section.'_800x638_'.date('Y_m_d_H_i_s').'.'.$pic_type;
			$res = copy($old_pic, $copy_Thumb);
			$this->app->my_image_resize ( $copy_Thumb, $copy_Thumb, 800, 638 );
			$arr_pic['partner_pic_800'] = $copy_Thumb;
		
			$copy_Thumb = $uploaddir.'/'.$section.'_100x80_'.date('Y_m_d_H_i_s').'.'.$pic_type;
			$res = copy($old_pic, $copy_Thumb);
			$this->app->my_image_resize ( $copy_Thumb, $copy_Thumb, 100, 80 );
			$arr_pic['partner_pic_100'] = $copy_Thumb;

			$copy_Thumb = $uploaddir.'/'.$section.'_200x160_'.date('Y_m_d_H_i_s').'.'.$pic_type;
			$res = copy($old_pic, $copy_Thumb);
			$this->app->my_image_resize ( $copy_Thumb, $copy_Thumb, 200, 160 );
			$arr_pic['partner_pic_200'] = $copy_Thumb;
		
			$copy_Thumb = $uploaddir.'/'.$section.'_400x319_'.date('Y_m_d_H_i_s').'.'.$pic_type;
			$res = copy($old_pic, $copy_Thumb);
			$this->app->my_image_resize ( $copy_Thumb, $copy_Thumb, 400, 319 );
			$arr_pic['partner_pic_400'] = $copy_Thumb;

			$copy_Thumb = $uploaddir.'/'.$section.'_600x479_'.date('Y_m_d_H_i_s').'.'.$pic_type;
			$res = copy($old_pic, $copy_Thumb);
			$this->app->my_image_resize ( $copy_Thumb, $copy_Thumb, 600, 479 );
			$arr_pic['partner_pic_600'] = $copy_Thumb;
			
			
			$this->PartnerModel->edit_partner($partner_id, $arr_pic);
		}
		//----修改图片路径--END-----//
	...
	//修改合作伙伴
	function toedit_partner($partner_id){
		//跳转到列表页面
		...
		//导航栏
		if($this->langtype == '_ch'){
			$data['url'] = '<a href="'.$decodebackurl.'">管理合作伙伴</a> &gt; 修改合作伙伴';
		}else{
			$data['url'] = '<a href="'.$decodebackurl.'">Manage partner</a> &gt; Edit partner';
		}
		
		$data['partnerinfo']=$this->PartnerModel->getpartnerinfo($partner_id);
		$this->load->view('admin/partner_edit',$data);
	}
	//修改合作伙伴 ------- 处理方法
	function edit_partner($partner_id){
		$oldpartnerinfo = $this->PartnerModel->getpartnerinfo($partner_id);
		...
		$this->PartnerModel->edit_partner($partner_id, $arr);
	...
	function del_partner($partner_id){
		$this->PartnerModel->del_partner($partner_id);
	}
	
	//合作伙伴列表 -- 排序功能
	function editpartner_sort(){
		$idarr=$this->input->post('idarr');
		$newsrot=$this->input->post('newsrot');
		if(!empty($idarr)){
			for($i=0;$i<count($idarr);$i++){
				$arr = array('sort'=>$newsrot[$i]);
				$this->PartnerModel->edit_partner($idarr[$i], $arr);
			}
		}
	}
