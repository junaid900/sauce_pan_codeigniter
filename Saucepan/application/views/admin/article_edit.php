...
<script type="text/javascript" src='<?php echo CDN_URL();?>themes/default/js/admin/admin_article.js?date=<?php echo CACHE_USETIME()?>'></script>
...
<form method="post">
	<table class="gksel_normal_tabpost">
		<tr>
			...
			<td>
				<script>
					function toviewarticleoriginal(path){
				...
				<div class="img_gksel_show" id="img1_gksel_show">
					<div style="float:left;">
						<?php 
							$img1_gksel = '';
							if(file_exists($articleinfo['article_pic_800']) && $articleinfo['article_pic_800']!=""){
								echo '<img style="float:left;max-width:400px;max-height:400px;" src="'.base_url().$articleinfo['article_pic_800'].'" />';
							}
							
							if(file_exists($articleinfo['article_pic_original']) && $articleinfo['article_pic_original']!=""){
								$img1_gksel = $articleinfo['article_pic_original'];
							}
						?>
					</div>
					<div style="float:left;margin-left:10px;">
						<?php if(file_exists($articleinfo['article_pic_original']) && $articleinfo['article_pic_original']!=""){?>
							<a href="javascript:;" onclick="toviewarticleoriginal('<?php echo $articleinfo['article_pic_original']?>')"><?php if($this->langtype == '_ch'){echo '查看原图';}else{echo 'View original photo';}?></a>
						<?php }?>
					</div>
				</div>
				...
				<div style="float:left;margin-left:5px;margin-top:5px;"><font class="fonterror" id="img1_gksel_error"><font style="color:gray;">仅支持 Jpg, Png, Gif 仅支持 Jpg, Png,jpeg, Gif 格式 (建议尺寸1000*700，图片需小于2M需可上传) </font></div>
			</td>
		</tr>
        			...
        			<td align="right" width="150"><?php if($this->langtype == '_ch'){echo '时间';}else{echo 'Time';}?></td>
        			  ...
        			  <td>
        			     <div class="form-group float-left w140" style="margin-bottom:0;">
                              <input type="text" name="datepicker"  id="datetimepicker3" class="form-control" value="<?php echo date('Y-m-d', $articleinfo['datepicker'])?>"/>
		...
		<?php for ($lc = 0; $lc < count($lancodelist); $lc++) {?>
			<tbody>
				<tr>
					<td align="right" width="150"><?php if($this->langtype == '_ch'){echo '关于我们名称';}else{echo 'About name';}?> <?php if(count($lancodelist) != 1){echo '('.$lancodelist[$lc]['langname'].')';}?></td>
					<td align="left">
						<input type="text" name="article_name<?php echo $lancodelist[$lc]['langtype']?>" style="width:300px;" value="<?php echo $articleinfo['article_name'.$lancodelist[$lc]['langtype']]?>"/>
        		...
        		<tr>
        			<td align="right" width="150"><?php if($this->langtype == '_ch'){echo '简介';}else{echo 'Tagline';}?></td>
        			  ...
        			  <td>
                          <textarea style="float:left;width:400px;height:50px;" name="article_tagline<?php echo $lancodelist[$lc]['langtype']?>"><?php echo $articleinfo['article_tagline'.$lancodelist[$lc]['langtype']]?></textarea>
        			  </td>
        		</tr>
				<tr>
					<td align="right" width="150"><?php echo lang('dz_article_description')?> <?php if(count($lancodelist) != 1){echo '('.$lancodelist[$lc]['langname'].')';}?></td>
					<td align="left">
						<script id="article_description<?php echo $lancodelist[$lc]['langtype']?>" type="text/plain" style="width:800px;height:300px;"><?php echo debaseurlcontent($articleinfo['article_description'. $lancodelist[$lc]['langtype']])?></script>
						<script type="text/javascript">
							var article_description<?php echo $lancodelist[$lc]['langtype']?> = UE.getEditor('article_description<?php echo $lancodelist[$lc]['langtype']?>');
						</script>
					</td>
				</tr>
				<tr>
		...
		<tr>
			...
			<td align="left">
				<input name="status" type="checkbox" defaultvalue="0" class="mgc-switch" value="1" <?php if($articleinfo['status'] == 1){echo 'checked';}?>/>
			</td>
		</tr>
		<tr>
			<td>
...
$(document).ready(function(){
	...
	if(button_gksel1.length>0){
		new AjaxUpload(button_gksel1,{
			action: baseurl+'index.php/welcome/uplogo/1000/700', 
	...
	function tosave_articleinfo(article_id){
		...
		if(isajaxsaveing == 0){
			...
			//----定义的字段------START
				...
				GETOBJ[GETOBJ_num]['field_realname'] = 'article_pic_original';
				GETOBJ[GETOBJ_num]['field_type'] = "image";
				GETOBJ[GETOBJ_num]['field_CMNAME'] = L['dz_article_picture'];
			...
			//----定义多语言的字段------START
				...
				GETLANGOBJ[GETLANGOBJ_num]['field_name'] = 'article_name';
				GETLANGOBJ[GETLANGOBJ_num]['field_type'] = "input";
				GETLANGOBJ[GETLANGOBJ_num]['field_CMNAME'] = L['dz_article_name'];

				GETLANGOBJ_num = GETLANGOBJ_num + 1;
				GETLANGOBJ[GETLANGOBJ_num] = new Array();
				GETLANGOBJ[GETLANGOBJ_num]['field_name'] = 'article_tagline';
				GETLANGOBJ[GETLANGOBJ_num]['field_type'] = "textarea";
				GETLANGOBJ[GETLANGOBJ_num]['field_CMNAME'] = L['dz_article_tagline'];

				GETLANGOBJ_num = GETLANGOBJ_num + 1;
				GETLANGOBJ[GETLANGOBJ_num] = new Array();
				GETLANGOBJ[GETLANGOBJ_num]['field_name'] = 'article_description';
				GETLANGOBJ[GETLANGOBJ_num]['field_type'] = "ueditor";
				GETLANGOBJ[GETLANGOBJ_num]['field_CMNAME'] = L['dz_article_description'];
			//----定义多语言的字段------END
			...
			if(returnOBJ.ispass == 1){
				$.post(baseurl+'index.php/admins/article/edit_article/'+article_id, postOBJ, function (data){
					var obj = eval( "(" + data + ")" );
