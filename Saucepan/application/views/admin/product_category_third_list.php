...
<table class="gksel_normal_tabaction">
	<tr>
		<td>
			<div class="searcharea">
				<a href="<?php echo base_url().'index.php/admins/product/toadd_product_thirdcategory/'.$parent.'/'.$second_id.'?backurl='.$backurl.'&subbackurl='.$subbackurl.'&thirdbackurl='.$current_url_encode?>">
					<font class="nav_on">
						<font class="plus">
						...
						<span><?php echo lang('dz_product_thirdcategory_add')?></span>
					</font>
				</a>
			</div>
		</td>
...
<ul id="tasks" style="float:left;width:100%;padding:0px;margin:0px 0px 0px 0px;list-style-type:none;">
	<?php if(isset($thirdcategorylist)){for ($i = 0; $i < count($thirdcategorylist); $i++) {?>
		<li class="articlelist" id="<?php echo $thirdcategorylist[$i]['category_id']?>" iid="<?php echo $i+1?>" style="width:100%;padding:0px;margin:0px;list-style-type: none;">
			<table class="gksel_normal_tablist" style="margin-top: 0px;">
				<tbody>
					<tr style="<?php if($thirdcategorylist[$i]['status'] == 0){echo 'opacity:0.2;';}?>">
						<td width="50" align="center" style="padding:0px;"><?php echo ($i+1)?></td>
						<td>
							<div style="float:left;width:100%;">
								<?php echo actionsearchdaxiaoxiezimu($keyword, strip_tags($thirdcategorylist[$i]['category_name'.$this->langtype]));?>
							</div>
							<div style="float:left;width:100%;color:#CCC;">
								<?php echo base_url().'index.php/wechat/product?category_id='.$thirdcategorylist[$i]['parent'].'&subcategory_id='.$thirdcategorylist[$i]['category_id']?>
							</div>
						</td>
						<td width="165" align="center" style="padding:0px;"><?php echo date('Y-m-d H:i:s', $thirdcategorylist[$i]['edited'])?></td>
						<td width="100" align="center" style="padding:0px;"><?php echo $thirdcategorylist[$i]['edited_author']?></td>
						<td width="100" align="center" style="padding:0px;">
							<?php 
								if($thirdcategorylist[$i]['status'] == 1){
									echo lang('cy_online');
								}else{
						...
						<td width="300" align="center" style="padding:0px;">
							<div style="float:right;">
								<?php 
									echo '<a href="'.base_url().'index.php/admins/product/toedit_productthirdcategory/'.$parent.'/'.$second_id.'/'.$thirdcategorylist[$i]['category_id'].'?backurl='.$backurl.'&subbackurl='.$subbackurl.'&thirdbackurl='.$current_url_encode.'" class="gksel_btn_action_on">'.lang('cy_edit').'</a>';
									$con = array('category_id'=>$parent, 'subcategory_id'=>$second_id, 'thirdcategory_id'=>$thirdcategorylist[$i]['category_id']);
									$count = $this->ProductModel->getproductlist($con, 1);
									if($count > 0){
										echo '<a title="该分类有'.$count.'个产品, 不能删除" href="javascript:;" class="gksel_btn_action_off">'.lang('cy_delete').'</a>';
									}else{
										echo '<a onclick="todel_productcategory('.$thirdcategorylist[$i]['category_id'].', \''.$thirdcategorylist[$i]['category_name'.$this->langtype].'\')" href="javascript:;" class="gksel_btn_action_on">'.lang('cy_delete').'</a>';
									}
								?>
							</div>
						</td>
