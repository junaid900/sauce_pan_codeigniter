...
<form method="post">
	<table class="gksel_normal_tabpost">
		<tr>
			<td align="right"><?php if($this->langtype == '_ch'){echo '设计师头像';}else{echo 'Designer avatar';}?></td>
			<td>
				...
				<div style="float:left;margin-left:5px;margin-top:5px;"><font class="fonterror" id="img1_gksel_error"><font style="color:gray;">仅支持 Jpg, Png, Gif 格式 (1000px * 1000px)</font></div>
			</td>
		</tr>
		<tr style="display:none;">
			<td align="right" width="150">优势</td>
			<td align="left">
				<input type="text" name="designerone_advantage" style="width:300px;" value=""/>
		...
		<tr style="display:none;">
			...
			<td align="left">
				<input type="text" name="designerone_motto" style="width:300px;" value=""/>
				<div class="tipsgroupbox"></div>
			</td>
		</tr>
		<tr>
		...
		<?php for ($lc = 0; $lc < count($lancodelist); $lc++) {?>
			<tr>
				...
				<td align="left">
					<input type="text" name="designerone_name<?php echo $lancodelist[$lc]['langtype']?>" style="width:300px;" value=""/>
					<div class="tipsgroupbox"></div>
				</td>
			</tr>
			<tr>
				...
				<td align="left">
					<input type="text" name="designerone_position<?php echo $lancodelist[$lc]['langtype']?>" style="width:300px;" value=""/>
					<div class="tipsgroupbox"></div>
				</td>
			</tr>
			<tr>
				<td align="right" width="150"><?php if($this->langtype == '_ch'){echo '设计师描述';}else{echo 'News Description';}?> <?php if(count($lancodelist) != 1){echo '('.$lancodelist[$lc]['langname'].')';}?></td>
				<td align="left">
					<script id="designerone_description<?php echo $lancodelist[$lc]['langtype']?>" type="text/plain" style="width:800px;height:300px;"></script>
		...
		<tr>
			...
			<td align="left">
				<div class="gksel_btn_action_on" onclick="toadd_designeroneinfo()"><?php echo lang('cy_save')?></div>
			</td>
		</tr>
	</table>
</form>
...
$(document).ready(function(){
	...
	//商品信息---添加
	function toadd_designeroneinfo(){
		var lancodelist = getlancodelist();
		if(isajaxsaveing == 0){
			//具体点击的按钮
			actionsubmit_button = $('div[onclick="toadd_designeroneinfo()"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
			...
			if(returnOBJ.ispass == 1){
				$.post(baseurl+'index.php/admins/designerone/add_designerone', postOBJ, function (data){
					var obj = eval( "(" + data + ")" );
