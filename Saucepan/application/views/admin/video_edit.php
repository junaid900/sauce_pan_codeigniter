...
<form method="post">
	<table class="gksel_normal_tabpost">
		<tr>
			...
			<td>
				<script>
					function toviewvideooriginal(path){
				...
				<div class="img_gksel_show" id="img1_gksel_show">
					<div style="float:left;">
						<?php
							$img1_gksel = '';
							if(file_exists($videoinfo['video_pic']) && $videoinfo['video_pic']!=""){
								echo '<img style="float:left;max-width:400px;max-height:400px;" src="'.base_url().$videoinfo['video_pic'].'" />';
							}

							if(file_exists($videoinfo['video_pic']) && $videoinfo['video_pic']!=""){
								$img1_gksel = $videoinfo['video_pic'];
							}
						?>
					</div>
					<div style="float:left;margin-left:10px;">
						<?php if(file_exists($videoinfo['video_pic']) && $videoinfo['video_pic']!=""){?>
							<a href="javascript:;" onclick="toviewvideooriginal('<?php echo $videoinfo['video_pic']?>')"><?php if($this->langtype == '_ch'){echo '查看原图';}else{echo 'View original photo';}?></a>
						<?php }?>
					</div>
				</div>
        			...
        			<td align="right" width="150"><?php if($this->langtype == '_ch'){echo '时间';}else{echo 'Time';}?></td>
        			  ...
        			  <td>
        			     <div class="form-group float-left w140" style="margin-bottom:0;">
                              <input type="text" name="datepicker"  id="datetimepicker3" class="form-control" value="<?php echo date('Y-m-d', $videoinfo['datepicker'])?>"/>
		...
		<?php for ($lc = 0; $lc < count($lancodelist); $lc++) {?>
			<tbody>
				<tr>
					<td align="right" width="150"><?php if($this->langtype == '_ch'){echo '活动名称';}else{echo 'video name';}?> <?php if(count($lancodelist) != 1){echo '('.$lancodelist[$lc]['langname'].')';}?></td>
					<td align="left">
						<input type="text" name="video_name<?php echo $lancodelist[$lc]['langtype']?>" style="width:300px;" value="<?php echo $videoinfo['video_name'.$lancodelist[$lc]['langtype']]?>"/>
        		...
        		<tr>
        			<td align="right" width="150"><?php if($this->langtype == '_ch'){echo '简介';}else{echo 'Tagline';}?></td>
        			  ...
        			  <td>
                          <textarea style="float:left;width:400px;height:50px;" name="video_tagline<?php echo $lancodelist[$lc]['langtype']?>"><?php echo $videoinfo['video_tagline'.$lancodelist[$lc]['langtype']]?></textarea>
        			  </td>
        		</tr>
				<tr>
					<td align="right" width="150"><?php echo lang('dz_video_description')?> <?php if(count($lancodelist) != 1){echo '('.$lancodelist[$lc]['langname'].')';}?></td>
					<td align="left">
						<script id="video_description<?php echo $lancodelist[$lc]['langtype']?>" type="text/plain" style="width:800px;height:300px;"><?php echo debaseurlcontent($videoinfo['video_description'. $lancodelist[$lc]['langtype']])?></script>
						<script type="text/javascript">
							var video_description<?php echo $lancodelist[$lc]['langtype']?> = UE.getEditor('video_description<?php echo $lancodelist[$lc]['langtype']?>');
						</script>
					</td>
				</tr>
				<tr>
		...
		<tr>
			...
			<td align="left">
				<input name="status" type="checkbox" defaultvalue="0" class="mgc-switch" value="1" <?php if($videoinfo['status'] == 1){echo 'checked';}?>/>
			</td>
		</tr>
		<tr>
			<td>
			...
			<td align="left">
				<div class="gksel_btn_action_on" onclick="tosave_videoinfo(<?php echo $videoinfo['video_id']?>)"><?php echo lang('cy_save')?></div>
			</td>
		</tr>
	</table>
</form>
...
$(document).ready(function(){
	...
	//商品信息---保存
	function tosave_videoinfo(video_id){
		var lancodelist = getlancodelist();
		if(isajaxsaveing == 0){
			//具体点击的按钮
			actionsubmit_button = $('div[onclick="tosave_videoinfo('+video_id+')"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
			...
			//----定义的字段------START
				...
				GETOBJ[GETOBJ_num]['field_realname'] = 'video_pic';
				GETOBJ[GETOBJ_num]['field_type'] = "image";
				GETOBJ[GETOBJ_num]['field_CMNAME'] = L['dz_video_picture'];
			...
			//----定义多语言的字段------START
				...
				GETLANGOBJ[GETLANGOBJ_num]['field_name'] = 'video_name';
				GETLANGOBJ[GETLANGOBJ_num]['field_type'] = "input";
				GETLANGOBJ[GETLANGOBJ_num]['field_CMNAME'] = L['dz_video_name'];

				GETLANGOBJ_num = GETLANGOBJ_num + 1;
				GETLANGOBJ[GETLANGOBJ_num] = new Array();
				GETLANGOBJ[GETLANGOBJ_num]['field_name'] = 'video_tagline';
				GETLANGOBJ[GETLANGOBJ_num]['field_type'] = "textarea";
				GETLANGOBJ[GETLANGOBJ_num]['field_CMNAME'] = L['dz_video_tagline'];

				GETLANGOBJ_num = GETLANGOBJ_num + 1;
				GETLANGOBJ[GETLANGOBJ_num] = new Array();
				GETLANGOBJ[GETLANGOBJ_num]['field_name'] = 'video_description';
				GETLANGOBJ[GETLANGOBJ_num]['field_type'] = "ueditor";
				GETLANGOBJ[GETLANGOBJ_num]['field_CMNAME'] = L['dz_video_description'];
			//----定义多语言的字段------END
			...
			if(returnOBJ.ispass == 1){
				$.post(baseurl+'index.php/admins/video/edit_video/'+video_id, postOBJ, function (data){
					var obj = eval( "(" + data + ")" );
