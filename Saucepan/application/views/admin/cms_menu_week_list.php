	...
	<tr>
		<td>
			...
			<div style="float:right;margin-right:10px;">
				<div onclick="javascript:location.href='<?php echo base_url().'index.php/admins/cms/menuweeklist/'.$menu_id?>';" style="cursor:pointer;float:left;margin-left:5px;margin-top:-4px;background:rgb(171,16,50);color:white;font-size:12px;padding:6px 8px;">
					Live weeks <span style="color:green;"><?php echo '('.$count_live.')';?></span>
				</div>
				<div onclick="javascript:location.href='<?php echo base_url().'index.php/admins/cms/menuweek_expiratedlist/'.$menu_id?>';" style="cursor:pointer;float:left;margin-left:5px;margin-top:-4px;background:#CCC;color:gray;font-size:12px;padding:6px 8px;">
...
<?php 
	$sql = "SELECT * FROM ".DB_PRE()."system_week_list WHERE week_id >= ".$current_week_id." ORDER BY week_end ASC";
		...
		<?php if(!empty($weeklist)){for ($i = 0; $i < count($weeklist); $i++) {?>
			<?php 
				...
				$sql = "
					...
					FROM ".DB_PRE()."menu_box 
					
					WHERE menu_id = ".$menu_id." AND week_id = ".$weeklist[$i]['week_id']." AND status = 1
				";
				$islivethisweek_res = $this->db->query($sql)->row_array();
			?>		
			<tr <?php if(!empty($islivethisweek_res)){}else{echo 'style="opacity:0.2;"';}?>>
			...
			<?php for ($j = 1; $j <= 7; $j++) {?>
				<?php 
					...
					if(empty($boxinfo)){
						$arr = array('menu_id'=>$menu_id, 'week_id'=>$weeklist[$i]['week_id'], 'weekday_num'=>$j, 'status'=>0, 'created'=>mktime(), 'edited'=>mktime());
						$this->db->insert(DB_PRE().'menu_box', $arr);
				...
				<tr <?php if(!empty($islivethisweek_res)){}else{echo 'style="opacity:0.2;"';}?>>
					...
					<td <?php if($j != 7){echo 'style="border-bottom:1px solid #CCC;padding-top:5px;padding-bottom:5px;"';}else{echo 'style="padding-top:5px;padding-bottom:5px;"';}?>" align="center">
						<div style="float:right;">
							<?php 
								echo '<a href="'.base_url().'index.php/admins/cms/toedit_weekdaybox/'.$menu_id.'/'.$weeklist[$i]['week_id'].'/'.$j.'" class="gksel_btn_action_on">Edit</a>';
							?>
						</div>
					</td>
				</tr>
