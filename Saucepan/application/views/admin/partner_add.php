...
<form method="post">
	<table class="gksel_normal_tabpost">
		<tr>
			<td align="right"><?php if($this->langtype == '_ch'){echo '合作伙伴图片';}else{echo 'News picture';}?></td>
			<td>
		...
		<?php for ($lc = 0; $lc < count($lancodelist); $lc++) {?>
			<tr>
				<td align="right" width="150"><?php if($this->langtype == '_ch'){echo '合作伙伴标题';}else{echo 'News Title';}?> <?php if(count($lancodelist) != 1){echo '('.$lancodelist[$lc]['langname'].')';}?></td>
				<td align="left">
					<input type="text" name="partner_name<?php echo $lancodelist[$lc]['langtype']?>" style="width:300px;" value=""/>
					<div class="tipsgroupbox"></div>
				</td>
			</tr>
			<tr style="display:none;">
				<td align="right" width="150"><?php if($this->langtype == '_ch'){echo '合作伙伴描述';}else{echo 'News Description';}?> <?php if(count($lancodelist) != 1){echo '('.$lancodelist[$lc]['langname'].')';}?></td>
				<td align="left">
					<script id="partner_description<?php echo $lancodelist[$lc]['langtype']?>" type="text/plain" style="width:800px;height:300px;"></script>
		...
		<tr>
			...
			<td align="left">
				<div class="gksel_btn_action_on" onclick="toadd_partnerinfo()"><?php echo lang('cy_save')?></div>
			</td>
		</tr>
	</table>
</form>
...
$(document).ready(function(){
	...
	//商品信息---添加
	function toadd_partnerinfo(){
		var lancodelist = getlancodelist();
		if(isajaxsaveing == 0){
			//具体点击的按钮
			actionsubmit_button = $('div[onclick="toadd_partnerinfo()"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
			...
			if(returnOBJ.ispass == 1){
				$.post(baseurl+'index.php/admins/partner/add_partner', postOBJ, function (data){
					var obj = eval( "(" + data + ")" );
