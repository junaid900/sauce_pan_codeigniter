	...
	<table class="gksel_normal_tabpost">
		...
		<?php for ($lc = 0; $lc < count($lancodelist); $lc++) {?>
			<tr>
				...
				<td align="left">
					<input type="text" name="news_name<?php echo $lancodelist[$lc]['langtype']?>" style="width:300px;" value=""/>
			...
			<tr>
				...
				<td align="left">
					<script id="news_description<?php echo $lancodelist[$lc]['langtype']?>" type="text/plain" style="width:800px;height:300px;"></script>
		...
		<tr>
			...
			<td align="left">
				<div class="gksel_btn_action_on" onclick="toadd_newsinfo()"><?php echo lang('cy_save')?></div>
			</td>
		</tr>
	</table>
</form>
...
$(document).ready(function(){
	...
	//商品信息---添加
	function toadd_newsinfo(){
		var lancodelist = getlancodelist();
		if(isajaxsaveing == 0){
			//具体点击的按钮
			actionsubmit_button = $('div[onclick="toadd_newsinfo()"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
			...
			if(returnOBJ.ispass == 1){
				$.post(baseurl+'index.php/admins/news/add_news', postOBJ, function (data){
					var obj = eval( "(" + data + ")" );
