...
<form method="post">
	<table class="gksel_normal_tabpost">
		...
		<?php for ($lc = 0; $lc < count($lancodelist); $lc++) {?>
			<tbody>
				<tr>
					...
					<td align="left">
						<input type="text" name="event_name<?php echo $lancodelist[$lc]['langtype']?>" style="width:300px;" value=""/>
				...
				<tr>
        			<td align="right" width="150"><?php if($this->langtype == '_ch'){echo '简介';}else{echo 'Tagline';}?></td>
        			  ...
        			  <td>
                          <textarea style="float:left;width:400px;height:50px;" name="event_tagline<?php echo $lancodelist[$lc]['langtype']?>"></textarea>
        			  </td>
        		</tr>
				<tr>
					...
					<td align="left">
						<script id="event_description<?php echo $lancodelist[$lc]['langtype']?>" type="text/plain" style="width:800px;height:300px;"></script>
		...
		<tr>
			...
			<td align="left">
				<div class="gksel_btn_action_on" onclick="toadd_eventinfo()"><?php echo lang('cy_save')?></div>
			</td>
		</tr>
	</table>
</form>
...
$(document).ready(function(){
	...
	//商品信息---添加
	function toadd_eventinfo(){
		var lancodelist = getlancodelist();
		if(isajaxsaveing == 0){
			//具体点击的按钮
			actionsubmit_button = $('div[onclick="toadd_eventinfo()"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
			...
			if(returnOBJ.ispass == 1){
				$.post(baseurl+'index.php/admins/event/add_event', postOBJ, function (data){
					var obj = eval( "(" + data + ")" );
