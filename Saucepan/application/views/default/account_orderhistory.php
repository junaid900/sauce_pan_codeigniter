...
<div style="width: calc(100% - 20px);padding:0 10px;margin:0px 0;float: left;margin-bottom: 0;" class="paage_box">
</div>
	    ...
	    //函数参数 "data" 为请求成功服务端返回的数据
			...
			if(timestamp<=timestamp1){
				...
				$.ajax({
				 ...
				 type: 'POST',
				 url: "https://www.mygkselss.com/market/apis/get_all_orders",
				 cache:false,
				 dataType: "json", 
				 data: formData,
				 processData: false,
				 ...
				 success: function(data){
				    ...
				    console.log("我的订单",data)
						for(var i=0;i<data.response.orders.length;i++){
							var order = data.response.orders[i];
							...
							//var orderType = (order.order_day == "today" || order.order_day == "Today") && deliveryTime == order.order_time  ?"":" ( PREORDER 预约 )";
							if(order.preorder != undefined){
							    orderType = (order.preorder == "0") ?"":"PREORDER 预约";
							}
							
							if(data.response.orders[i].payment_status=='Paid'){
						...
						'</div>'+
						'<p class="col3">'+<?php if($this->langtype == '_ch'){echo 'data.response.orders[i].products[j].attributes[z].product_attribute_title_ch';}else{echo 'data.response.orders[i].products[j].attributes[z].product_attribute_title_en';}?>+':</p>'+
						...
						'</div>'+
						'<p class="col4 itemName">'+<?php if($this->langtype == '_ch'){echo 'data.response.orders[i].products[j].product_name_ch';}else{echo 'data.response.orders[i].products[j].product_name_en';}?>+'<br>'+'<span style="color:#00ac00">x '+data.response.orders[i].products[j].qty+'</span>'+'</p>'+
						'<p class="col4 itemPrice align-right"><span style="opacity:0">¥'+<?php if($this->langtype == '_ch'){echo 'data.response.orders[i].products[j].product_name_ch';}else{echo 'data.response.orders[i].products[j].product_name_en';}?>+'</span><br>'+'<span style="color:#00ac00">x '+data.response.orders[i].products[j].total+'</span>'+'</p>'+
					'</div>'+
					'<div class="row direction-row" style="flex-wrap: wrap;">'+
						line+
					'</div>';
							}
							$(".card-list").append('<li class="orderObject card"><div class="card-head"><div class="row direction-row"><p class="title col2"></p><!-- <p class="col1 orderStatus subtitle align-right">Awaiting Payment| <a href="continuePayment/892956">Pay Now</a> </p> --></div><div class="row orderPhone direction-row"><p style="width:78px">Order date:</p><p class="col5"><b>'+data.response.orders[i].created_at+'</b></p></div><div class="row orderPhone direction-row"><p style="width:105px">Delivery Time:</p><p class="col5"><b>'+deliveryDate+' / '+deliveryTime+'<span style="color:red;font-size:12px;">'+orderType+'<span></b></p></div><div class="row orderPhone direction-row"><p style="width:107px">Payment Status:</p><p class="col5"><b>'+data.response.orders[i].payment_status+'</b></p></div><div class="row orderPhone direction-row"><p style="width:107px">SP points used:</p><p class="col5"><b>'+data.response.orders[i].sp_points+'</b></p></div><div class="row orderPhone direction-row"><p style="width:107px">Order Status:</p><p class="col5"><b>'+data.response.orders[i].order_status+'</b></p></div><div class="row orderPhone direction-row"><p style="width:68px">Order No:</p><p class="col5"> <b> '+data.response.orders[i].order_id+'</b></p></div><div class="row orderPhone direction-row"><p style="width:68px">Phone:</p><p class="col5"><b>'+data.response.orders[i].contact+'</b></p></div><div class="row orderAddress direction-row"><p style="width:68px">Address:</p><p class="col5"><b>'+data.response.orders[i].c_address+'</b></p></div><br><div class="orderItems card-body">'+box+'<hr><div class="row direction-row"><p class="col5">Total (delivery fee inclusive) : </p><span class="orderPrice align-right col1"> ¥'+data.response.orders[i].grand_total+'</span></div></div></div><div class="card-footer" ><div class="row" style="width:100%;"><div style="width:100%;" class="button_invoice_area_'+data.response.orders[i].order_id+'"></div></div><div class="row"><a href="" class="button medium button_success" style="display:none">Track Order</a></div></div></li>')
						}
		...
		if(totalnum >= 1){
			...
			$.post(baseurl+'index.php/welcome/loading_invoice_button/'+order_id_group[num]+'/'+order_price_group[num], function (data){
				...
				if(totalnum == (num + 1)){
					...
					setTimeout('goto_loading_invoice_button('+(num + 1)+')', 100);
				}
			})
		}
	}
