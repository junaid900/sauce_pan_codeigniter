	//删除商品
	function todel_brand(id, name){
		var title = '您确定要删除展示展览<font style="color:red;">【'+name+'】</font>吗？';
		var subtitle = '';
		del_url = encodeURI(baseurl+"index.php/admins/brand/del_brand/"+id);
		todel(title, subtitle);
	}
	//删除商品分类
	function todel_brandcategory(id, name){
		var title = '您确定要删除商品分类<font style="color:red;">【'+name+'】</font>吗？';
		var subtitle = '';
		del_url = encodeURI(baseurl+"index.php/admins/brand/del_brandcategory/"+id);
		todel(title, subtitle);
	}
	//删除商品分类
	function todel_brandpicture(id, name){
		var title = '您确定要删除图片<font style="color:red;">【'+name+'】</font>吗？';
		var subtitle = '';
		del_url = encodeURI(baseurl+"index.php/admins/brand/del_brandpicture/"+id);
		todel(title, subtitle);
	}
	//商品第三级分类---添加
	function toadd_brandthirdcategoryinfo(parent, second_id){
		var lancodelist = getlancodelist();
		if(isajaxsaveing == 0){
			//具体点击的按钮
			actionsubmit_button = $('div[onclick="toadd_brandthirdcategoryinfo('+parent+', '+second_id+')"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
			...
			//----定义多语言的字段------START
				...
				GETLANGOBJ[GETLANGOBJ_num]['field_CMNAME'] = L['dz_brand_category_name'];
			//----定义多语言的字段------END
			...
			if(returnOBJ.ispass == 1){
				$.post(baseurl+'index.php/admins/brand/add_brand_thirdcategory/'+parent+'/'+second_id, postOBJ, function (data){
					var obj = eval( "(" + data + ")" );
	...
	//商品第三级分类---保存
	function tosave_brandthirdcategoryinfo(parent, second_id, category_id){
		var lancodelist = getlancodelist();
		if(isajaxsaveing == 0){
			//具体点击的按钮
			actionsubmit_button = $('div[onclick="tosave_brandthirdcategoryinfo('+parent+', '+second_id+', '+category_id+')"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
			var backurl = $('input[name="backurl"]').val();
			...
			postOBJ.GETLANGOBJ = returnOBJ.GETLANGOBJ_arr;
			if(returnOBJ.ispass == 1){
				$.post(baseurl+'index.php/admins/brand/edit_brandthirdcategory/'+parent+'/'+second_id+'/'+category_id, postOBJ, function (data){
					var obj = eval( "(" + data + ")" );
					actionsubmit_button.html('<img class="icon_success" src="'+baseurl+'themes/default/images/global_ok.png"/><span>'+L['cy_save_success']+'</span>');
					location.href = obj.thirdbackurl;
				})
	...
	//商品子分类---添加
	function toadd_brandsubcategoryinfo(parent){
		var lancodelist = getlancodelist();
		if(isajaxsaveing == 0){
			//具体点击的按钮
			actionsubmit_button = $('div[onclick="toadd_brandsubcategoryinfo('+parent+')"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
			var backurl = $('input[name="backurl"]').val();
			...
			postOBJ.GETLANGOBJ = returnOBJ.GETLANGOBJ_arr;
			if(returnOBJ.ispass == 1){
				$.post(baseurl+'index.php/admins/brand/add_brand_subcategory/'+parent, postOBJ, function (data){
					var obj = eval( "(" + data + ")" );
					actionsubmit_button.html('<img class="icon_success" src="'+baseurl+'themes/default/images/global_ok.png"/><span>'+L['cy_save_success']+'</span>');
	...
	//商品子分类---保存
	function tosave_brandsubcategoryinfo(parent, category_id){
		var lancodelist = getlancodelist();
		if(isajaxsaveing == 0){
			//具体点击的按钮
			actionsubmit_button = $('div[onclick="tosave_brandsubcategoryinfo('+parent+', '+category_id+')"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
			var backurl = $('input[name="backurl"]').val();
			...
			postOBJ.GETLANGOBJ = returnOBJ.GETLANGOBJ_arr;
			if(returnOBJ.ispass == 1){
				$.post(baseurl+'index.php/admins/brand/edit_brandsubcategory/'+parent+'/'+category_id, postOBJ, function (data){
					var obj = eval( "(" + data + ")" );
					actionsubmit_button.html('<img class="icon_success" src="'+baseurl+'themes/default/images/global_ok.png"/><span>'+L['cy_save_success']+'</span>');
					location.href = obj.subbackurl;
				})
	...
	//商品分类---添加
	function toadd_brandcategoryinfo(){
		var lancodelist = getlancodelist();
		if(isajaxsaveing == 0){
			//具体点击的按钮
			actionsubmit_button = $('div[onclick="toadd_brandcategoryinfo()"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
			var backurl = $('input[name="backurl"]').val();
			...
			postOBJ.GETLANGOBJ = returnOBJ.GETLANGOBJ_arr;
			if(returnOBJ.ispass == 1){
				$.post(baseurl+'index.php/admins/brand/add_brand_category', postOBJ, function (data){
					var obj = eval( "(" + data + ")" );
					actionsubmit_button.html('<img class="icon_success" src="'+baseurl+'themes/default/images/global_ok.png"/><span>'+L['cy_save_success']+'</span>');
	...
	//商品分类---保存
	function tosave_brandcategoryinfo(category_id){
		var lancodelist = getlancodelist();
		if(isajaxsaveing == 0){
			//具体点击的按钮
			actionsubmit_button = $('div[onclick="tosave_brandcategoryinfo('+category_id+')"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
			var backurl = $('input[name="backurl"]').val();
			...
			postOBJ.GETLANGOBJ = returnOBJ.GETLANGOBJ_arr;
			if(returnOBJ.ispass == 1){
				$.post(baseurl+'index.php/admins/brand/edit_brandcategory/'+category_id, postOBJ, function (data){
					var obj = eval( "(" + data + ")" );
					actionsubmit_button.html('<img class="icon_success" src="'+baseurl+'themes/default/images/global_ok.png"/><span>'+L['cy_save_success']+'</span>');
					location.href = obj.backurl;
				})
	...
	function toadd_brandinfo(){
		...
		if(isajaxsaveing == 0){
			...
			//----定义的字段------START
				...
				GETOBJ[GETOBJ_num]['field_realname'] = 'brand_pic_original';
				GETOBJ[GETOBJ_num]['field_type'] = "image";
				GETOBJ[GETOBJ_num]['field_CMNAME'] = L['dz_brand_picture'];
	
				GETOBJ_num = GETOBJ_num + 1;
				GETOBJ[GETOBJ_num] = new Array();
				GETOBJ[GETOBJ_num]['field_name'] = 'brand_SKUno';
				GETOBJ[GETOBJ_num]['field_realname'] = 'brand_SKUno';
				GETOBJ[GETOBJ_num]['field_type'] = "input";
				GETOBJ[GETOBJ_num]['field_CMNAME'] = '产品编号';

				GETOBJ_num = GETOBJ_num + 1;
				GETOBJ[GETOBJ_num] = new Array();
				GETOBJ[GETOBJ_num]['field_name'] = 'brand_size';
				GETOBJ[GETOBJ_num]['field_realname'] = 'brand_size';
				GETOBJ[GETOBJ_num]['field_type'] = "input";
				GETOBJ[GETOBJ_num]['field_CMNAME'] = '尺寸';

				GETOBJ_num = GETOBJ_num + 1;
				GETOBJ[GETOBJ_num] = new Array();
				GETOBJ[GETOBJ_num]['field_name'] = 'brand_weight_1';
				GETOBJ[GETOBJ_num]['field_realname'] = 'brand_weight_1';
				GETOBJ[GETOBJ_num]['field_type'] = "input";
				GETOBJ[GETOBJ_num]['field_CMNAME'] = '重量';

				GETOBJ_num = GETOBJ_num + 1;
				GETOBJ[GETOBJ_num] = new Array();
				GETOBJ[GETOBJ_num]['field_name'] = 'brand_weight_2';
				GETOBJ[GETOBJ_num]['field_realname'] = 'brand_weight_2';
				GETOBJ[GETOBJ_num]['field_type'] = "input";
			...
			//----定义多语言的字段------START
				...
				GETLANGOBJ[GETLANGOBJ_num]['field_type'] = "input";
				GETLANGOBJ[GETLANGOBJ_num]['field_CMNAME'] = L['dz_brand_name'];
				
				GETLANGOBJ_num = GETLANGOBJ_num + 1;
				GETLANGOBJ[GETLANGOBJ_num] = new Array();
				GETLANGOBJ[GETLANGOBJ_num]['field_name'] = 'brand_tagline';
				GETLANGOBJ[GETLANGOBJ_num]['field_type'] = "textarea";
				GETLANGOBJ[GETLANGOBJ_num]['field_CMNAME'] = L['dz_brand_tagline'];
	
				GETLANGOBJ_num = GETLANGOBJ_num + 1;
				GETLANGOBJ[GETLANGOBJ_num] = new Array();
				GETLANGOBJ[GETLANGOBJ_num]['field_name'] = 'brand_description';
				GETLANGOBJ[GETLANGOBJ_num]['field_type'] = "ueditor";
				GETLANGOBJ[GETLANGOBJ_num]['field_CMNAME'] = L['dz_brand_description'];
			//----定义多语言的字段------END
...
//				}
	...
	//商品信息---保存
	function tosave_brandinfo(brand_id){
		var lancodelist = getlancodelist();
		if(isajaxsaveing == 0){
			//具体点击的按钮
			actionsubmit_button = $('div[onclick="tosave_brandinfo('+brand_id+')"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
			var backurl = $('input[name="backurl"]').val();
...
//				}
			...
			if(returnOBJ.ispass == 1){
				$.post(baseurl+'index.php/admins/brand/edit_brand/'+brand_id, postOBJ, function (data){
					var obj = eval( "(" + data + ")" );
					actionsubmit_button.html('<img class="icon_success" src="'+baseurl+'themes/default/images/global_ok.png"/><span>'+L['cy_save_success']+'</span>');
	//				isajaxsaveing = 0;//ajax正在保存中 --- 释放
					location.href = obj.backurl;
